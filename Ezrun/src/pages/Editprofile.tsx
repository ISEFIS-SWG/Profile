...
import { home, backspace, arrowRoundBack, man,woman,checkmarkCircleOutline} from "ionicons/icons";
...
const Editprofille: React.FC<RouteComponentProps> = (props) => {
  ...
  const [pic, setpic] = useState("http://180.180.241.92/pic/profile/upload/cat.jpg");
  ...
  let takePicture = async() => {
    const image = await Camera.getPhoto({
      ...
      fromda.append('image',imageUrl);
      fromda.append('name',"user");
...
<IonSegmentButton value="woman" layout="icon-start"  checked={contact.Gender === 'female'}>
  ...
  let onSave = async() => {
    ...
    let result= await UpUesr(user_id,firstName,lastName,email,telnum,weight,heigth,homenum,
      mooban,moo,soi,buildname,floor,room,tumbon,aumper,province,zip,segment,fromda
      );
...
{name}
          ...
          <IonRow >
              <IonCol class="ion-text-center" >
                   <IonIcon  color="success"   class="comfr"  icon={checkmarkCircleOutline}  onClick={onSave}/>            
              </IonCol>
              <IonToast
        isOpen={showToast1}
        onDidDismiss={() => setShowToast1(false)}
        message="Your Update have been saved"
        duration={2000}
